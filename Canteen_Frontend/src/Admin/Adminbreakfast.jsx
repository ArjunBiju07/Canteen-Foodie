...
function Adminbreakfast() {
  ...
  const insertBreakfast = (e) => {
    e.preventDefault();

    fetch('http://localhost:3000/insertbreakfast', {
      method: 'POST',
      headers: {
  ...
  useEffect(() => {
    // Fetch existing breakfast items from the server if needed
    fetch('http://localhost:3000/getbreakfast') // Adjust the endpoint as per your backend setup
      .then(response => response.json())
      .then(data => {
        console.log("Breakfast items:", data);
        setBreakfastItems(data); // Assuming data is an array of breakfast items
      })
      .catch(error => {
          ...
          >
            <div className="modal-dialog">
              <div className="modal-content">
                <form onSubmit={insertBreakfast}>
                  <div className="modal-header">
                    <h5 className="modal-title" id="addOrderLabel">Add New Breakfast</h5>
                    <button
                      type="button"
                      className="btn-close"
